...
export default function Portal() {
  ...
  return (
    <div className="pt-32 pb-24 min-h-screen bg-dark">
      <Helmet>
        <title>Student Portal | Youth Taekwondo Academy</title>
        <meta
          name="description"
          content="Access your student records, track your progress, check belt rank, and view fee status at Elite Taekwondo Academy."
        />
      </Helmet>
              ...
              <div className="glass p-8 rounded-3xl border border-white/10 flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="md:ml-auto flex flex-col items-center md:items-end">
                  ...
                  <span className="text-2xl font-display text-primary">
                    {studentData.beltRank} Belt
                  </span>
                </div>
              </div>

              {/* Stats grid — all real data now */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  },
                  {
                    ...
                    value: `${studentData.marks ?? 0}%`,
                  },
                  {
                    icon: CreditCard,
                    label: 'Fees Pending',
                    value: `₹${studentData.feesPending ?? 0}`,
                    color:
                      (studentData.feesPending ?? 0) > 0 ? 'text-red-500' : 'text-green-500',
                  },
                  {
                    icon: User,
                    label: 'Attendance',
                  ...
                  <div key={idx} className="glass p-6 rounded-2xl border border-white/5">
                    ...
                    <p className={`text-xl font-display ${stat.color || 'text-white'}`}>
                      {stat.value}
                    </p>
                  </div>
                ))}
                ...
                <div className="space-y-6">
                  {(studentData.progress || []).length === 0 ? (
                    <p className="text-gray-500 text-sm italic py-4">
                      No progress records yet. Check back after your next exam or tournament!
                    </p>
                  ) : (
                    studentData.progress.map((item, idx) => (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <span
                          className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full ${getStatusStyle(
                            item.status
                          )}`}
                        >
                          {item.status}
